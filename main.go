...
func cmdInstallHook() {
	...
	settings["hooks"] = targetHooks

	out, _ := json.MarshalIndent(settings, "", "  ")
	os.WriteFile(settingsPath, append(out, '\n'), 0644)

	fmt.Println("Hooks installed in .claude/settings.json:")
	fmt.Println("  - PreToolUse: claudeignore guard (Read|Write|Edit|Grep|Glob)")
...
func cmdStatus() {
	...
	// Hook
	projectSettings := filepath.Join(root, ".claude", "settings.json")
	hookInstalled := false
	if data, err := os.ReadFile(projectSettings); err == nil {
		hookInstalled = strings.Contains(string(data), "claudeignore check")
	}
	if hookInstalled {
		fmt.Println("Detection hook: installed")
	} else {
		fmt.Println("Detection hook: not installed (run 'claudeignore install-hook')")
	}
}
...
Usage:
  ...
  claudeignore install-hook      Add hooks to .claude/settings.json
